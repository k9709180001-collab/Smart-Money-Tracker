...
export default function FiiDiiPanel({ fiiDii, onUpdateFiiDii, spotPrice }: FiiDiiPanelProps) {
  ...
  const getFlowSentiment = (net: number) => {
    ...
    if (net === 0) return { label: "Neutral", text: "text-slate-650 bg-slate-100 border-slate-200" };
    if (net > 500) return { label: "Strong Bullish", text: "text-emerald-700 bg-emerald-50 border-emerald-250" };
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm" id="fii-dii-card">
      <div className="px-6 py-4 bg-white border-b border-slate-200 flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <Shield className="text-indigo-650" size={18} />
          <h3 className="text-lg font-bold text-slate-850 font-sans tracking-tight">
            Institutional activity (FII / DII Flows)
          </h3>
        </div>
        <span className="text-slate-650 font-mono text-[10px] bg-slate-50 px-2.5 py-0.5 rounded border border-slate-200 font-bold">
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col justify-between">
            <div>
              <span className="text-xs text-slate-550 font-mono font-bold">FII Cash Net Flow</span>
              <div className="text-lg font-extrabold text-slate-850 mt-1 flex items-center gap-1.5">
                {fiiDii.fiiCashNet < 0 ? (
            ...
            <div className="mt-4">
              <input
                type="number"
                className="w-full bg-white border border-slate-250 rounded px-2 py-1 text-xs text-slate-850 font-mono focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-400"
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col justify-between">
            <div>
              <span className="text-xs text-slate-550 font-mono font-bold">DII Cash Net Flow</span>
              <div className="text-lg font-extrabold text-slate-850 mt-1 flex items-center gap-1.5">
                {fiiDii.diiCashNet < 0 ? (
                  <ArrowDownCircle className="text-red-600 flex-shrink-0" size={16} />
                ) : (
            ...
            <div className="mt-4">
              <input
                type="number"
                className="w-full bg-white border border-slate-250 rounded px-2 py-1 text-xs text-slate-850 font-mono focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-400"
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col justify-between">
            <div>
              <span className="text-xs text-slate-550 font-mono font-bold">FII Index Futures Net</span>
              <div className="text-lg font-extrabold text-slate-850 mt-1 flex items-center gap-1.5">
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col justify-between">
            <div>
              <span className="text-xs text-slate-550 font-mono font-bold">FII Index Options Net</span>
              <div className="text-lg font-extrabold text-slate-850 mt-1 flex items-center gap-1.5 text-indigo-700">
        ...
        <div className="p-4 rounded-lg bg-amber-50 border border-amber-200 flex gap-3 text-slate-705">
